...
// Carry-less multiplication (PCLMULQDQ) folding, following
//
...
//
// Folding a 128-bit chunk multiplies it by x^k mod P(x), advancing it past the
...
// For the CCITT polynomial. The paper tabulates all of these except the pair
// spanning 256 bits and the pair spanning 1024, which the 256-bit registers
// need: k1 = x^576, k2 = x^512, k3 = x^192, k4 = x^128, k5 = x^96,
// P' = 0x1db710641 and mu = 0x1f7011641.
...
static u32 CRCFoldTail(__m128i chunk, const unsigned char *current, size_t length)
{
  ...
  return CRCUpdateBlock_Slice4(CRCReduce(chunk), length, current);
}

PAR2_TARGET("sse2,pclmul")
static u32 CRCUpdateBlock_PclMul(u32 crc, size_t length, const void *buffer)
{
  const unsigned char *current = (const unsigned char *)buffer;

  if (length < 64)
    return CRCUpdateBlock_Slice4(crc, length, current);
...
PAR2_TARGET("avx2,pclmul,vpclmulqdq")
...
PAR2_TARGET("avx2,pclmul,vpclmulqdq")
static u32 CRCUpdateBlock_VPclMul(u32 crc, size_t length, const void *buffer)
{
  const unsigned char *current = (const unsigned char *)buffer;

  if (length < 128)
    return CRCUpdateBlock_PclMul(crc, length, buffer);

  __m256i y0 = _mm256_xor_si256(_mm256_loadu_si256((const __m256i *)current),
                                _mm256_castsi128_si256(_mm_cvtsi32_si128((int)crc)));
...
static bool X86HasVPclMul()
{
  ...
  CRCCpuId(1, 0, registers);
  const bool osxsave = (registers[2] & (1u << 27)) != 0;
  const bool avx = (registers[2] & (1u << 28)) != 0;
  if (!osxsave || !avx)
    return false;
  ...
  const u64 xcr0 = ((u64)upper << 32) | lower;
#else
  ...
  const bool avx2 = (registers[1] & (1u << 5)) != 0;
  const bool vpclmulqdq = (registers[2] & (1u << 10)) != 0;
  return avx2 && vpclmulqdq;
}
